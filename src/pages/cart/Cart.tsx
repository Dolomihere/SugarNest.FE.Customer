...
export function CartPage() {
  ...
  const { data } = useQuery({
    queryKey: ['cart'],
    queryFn: async () => {
      const res = await CartService.getGuestCart("01B2E2DF-61DF-4B3F-BC98-6A31A6D726FC");
                    ...
                    <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                      ...
                      <div className="flex items-center justify-between md:order-3 md:justify-end">
                        <div className="flex items-center">
                          ...
                          <input type="text" id="counter-input-5" data-input-counter className="w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white" placeholder="" value={item.quantity} onChange={handleQuantityChange} required />
                        ...
                        <div className="text-end md:order-4 md:w-32">
                          <p className="text-base font-bold text-gray-900 dark:text-white">{new Intl.NumberFormat('vi-VN').format(item.unitPrice ?? 0)} đ</p>
                        </div>
                      </div>

                      <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                        <a href="#" className="text-base font-medium text-gray-900 hover:underline dark:text-white">{item.productName}</a>
